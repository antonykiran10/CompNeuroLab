...
def du(u, m_, n_, h_, t, inp):
    return (input_current(t, inp) / Cm - (
            (u - ENa) * gNa * m_ * m_ * m_ * h_ + (u - EK) * gK * n_ * n_ * n_ * n_ + (u - EL) * gL) / Cm) * dt


def my_loop(current_looper):
    ...
    for i in range(1, N):
        ...
        membrane_voltage[i] = membrane_voltage[i - 1] + du(membrane_voltage[i - 1], m[i - 1], n[i - 1], h[i - 1],
                                                           time[i], current_looper)
        input_[i] = input_current(time[i - 1], current_looper)

    # print("--- %s seconds ---" % (clock.time() - start_time))

    df = pd.DataFrame({"Time (ms)": time, "Membrane Voltage (mV)": membrane_voltage, "m": m, "n": n, "h": h})
    df.to_csv(".\\run_data\\test\\output_" + str(current_looper) + "_current" + ".csv", index=False)


for c in range(0, 300):
    my_loop(c)
    print("Entered loop: ", c)
