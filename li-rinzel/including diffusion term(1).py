...
def diffsuion_term(c1, c2, c3,d):
    return ((c1 + c2 - (2 * c3)) / (dx * dx))*d*dt
...
if __name__ == '__main__':
    calcium_concentration = np.zeros((M, N + 1), float)
    calcium_concentration_ER = np.zeros((M, N + 1), float)
    h_vals = np.zeros((M, N), float)
    time = np.zeros(M, float)
    ip3_concentration = np.zeros((M, N), float)
    # m_infVals = np.zeros(M, float)
    # h_infVals = np.zeros(M, float)
    for k in range(0, N):
        calcium_concentration[0][k] = 0.035
        calcium_concentration_ER[0][k] = 50
        print(calcium_concentration
              )
        h_vals[0][k] = 0
        ip3_concentration[0][k] = 0.5
    ...
    for i in range(1, M):
        for j in range(1, N):
            ...
            calcium_concentration[i][j] = calcium_concentration[i - 1][j - 1] + dcal_cyto(
                calcium_concentration[i - 1][j - 1],
                calcium_concentration_ER[i - 1][j - 1],
                h_vals[i - 1][j - 1], ip3_concentration[i - 1][j - 1]) + diffsuion_term(
                calcium_concentration[i - 1][j + 1], calcium_concentration[i - 1][j - 1],
                calcium_concentration[i - 1][j], Dca)

            calcium_concentration_ER[i][j] = calcium_concentration_ER[i - 1][j - 1] + dcal_ER(
                calcium_concentration[i - 1][j - 1],
                calcium_concentration_ER[i - 1][j - 1],
                h_vals[i - 1][j - 1], ip3_concentration[i - 1][j - 1]) + diffsuion_term(
                calcium_concentration_ER[i - 1][j + 1], calcium_concentration_ER[i - 1][j - 1],
                calcium_concentration_ER[i - 1][j], Der)

            h_vals[i][j] = h_vals[i - 1][j - 1] + dh(h_vals[i - 1][j - 1], ip3_concentration[i - 1][j - 1],
                                                     calcium_concentration[i - 1][j - 1])

            # print(h_vals[i][j], calcium_concentration[i][j], calcium_concentration_ER[i][j])
            ip3_concentration[i][j] = ip3_input(time[i - 1])
    ...
    calcium_cyto = pd.DataFrame(calcium_concentration)
    calcium_cyto.to_csv('Calcium_cyto.csv')
    calcium_ER = pd.DataFrame(calcium_concentration)
    calcium_ER.to_csv('Calcium_ER.csv')
